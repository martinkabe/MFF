...
class NalezeneObjemy(object):
    ...
    def Vytiskni(cls):
        cls.seradit()
        for i in range(len(cls.objemy)):
            print(f"{cls.objemy[i]}:{cls.poctyKroku[i]}", end=" ")

    @classmethod
    def seradit(cls):
        for i in range(len(cls.objemy) - 1):
            for j in range(len(cls.objemy) - 1):
...
class Program(object):
    ...
    def Main(cls):
        lahve = []
        objemy, obsahy = DataReader.NactiData()
...
# 9 8 7  5 4 3
# 0:1 1:1 2:3 3:0 4:0 5:0 7:1 8:1 9:1